...
type boxPlatformInterfaceWrapper struct{ platformStub }
...
func (w *boxPlatformInterfaceWrapper) FindProcessInfo(ctx context.Context, network string, source netip.AddrPort, destination netip.AddrPort) (*adapter.ConnectionOwner, error) {
	...
	return connectionOwner(uid, packageName), nil
}
...
func (w *boxPlatformInterfaceWrapper) FindConnectionOwner(r *adapter.FindConnectionOwnerRequest) (*adapter.ConnectionOwner, error) {
	network := "tcp"
	if r.IpProtocol == syscall.IPPROTO_UDP {
		network = "udp"
	}
	source, err := netip.ParseAddr(r.SourceAddress)
	if err != nil {
		return nil, err
	}
	destination, err := netip.ParseAddr(r.DestinationAddress)
	if err != nil {
		return nil, err
	}
	info, err := w.FindProcessInfo(context.Background(), network, netip.AddrPortFrom(source, uint16(r.SourcePort)), netip.AddrPortFrom(destination, uint16(r.DestinationPort)))
	if err != nil {
		return nil, err
	}
	return info, nil
